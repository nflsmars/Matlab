...
%does not happen before maturity using Static Hedging Portfolios
%made up of european vanilla options and binary options
function price=DKISHP_ECONB_Rebate(S0,r,T,sigma,U,L,N)
...
h=D\(-V+BCU+BPL);
price=reshape([CUS;PLS],1,2*N)*h-ECONB(S0,U,r,T,sigma,1,1)-ECONB(S0,L,r,T,sigma,1,-1)+exp(-r*T);