%This is the function for pricing European Double Knock Out
...
function [price,std,CI]=DKO_MC_BB(S0,X,r,T,sigma,H,L,CorP,N_path)
N=64;
deltaT=T/N;
Stock=zeros(N_path,N+1);
Stock(:,1)=S0;
epsilon=randn(N_path,N);
value=zeros(N_path,1);
for i=2:N+1;
...
value(1:size(Stock_BB,1),1)=max(0,CorP*(Stock_BB(:,end)-X)).*...
