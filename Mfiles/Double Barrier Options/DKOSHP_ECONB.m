...
function price=DKOSHP_ECONB(S0,X,r,T,sigma,U,L,CorP,N)
a=max(CorP*(U-X),0);
b=max(CorP*(L-X),0);
...
if (X<=L&&CorP==-1)||(X>=U&&CorP==1)
    ...
    [c,d]=blsprice(S0,X,r,T,sigma);
    if CorP==1
        hC=D\(-CX+BCU+BPL);
        price=reshape([CUS;PLS],1,2*N)*hC+c-ECONB(S0,U,r,T,sigma,a,1)-ECONB(S0,L,r,T,sigma,b,-1);
    else
        hP=D\(-PX+BCU+BPL);
        price=reshape([CUS;PLS],1,2*N)*hP+d-ECONB(S0,U,r,T,sigma,a,1)-ECONB(S0,L,r,T,sigma,b,-1);
    end
end